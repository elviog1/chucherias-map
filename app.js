...
var womanIcon = L.icon({
  iconUrl: "../images/woman.png",
  iconSize: [32, 32], // Tamaño del icono [ancho, alto]
});
var workIcon = L.icon({
...
function createPopupForm(marker) {
  ...
  form +=
    '<input class="form-popup-input" placeholder="Titulo" type="text" name="title" required><br>';
  form +=
    '<br><textarea placeholder="Descripcion" name="description" rows="4" required maxlength="100"></textarea><br>';
  form +=
    'Categoría:<select class="form-popup-input" name="category"><option value="Comidas">Comidas</option><option value="Electricidad">Electricidad</option><option value="Hombre">Hombre</option><option value="Inseguridad">Inseguridad</option><option value="Mujer">Mujer</option><option value="Peligro">Peligro</option><option value="Trabajo">Trabajo</option><option value="Otros">Otros</option></select><br>';
  form +=
    '<div class="form-popup-buttons"><input class="popup-button save-button" type="submit" value="Guardar"><button class="popup-button delete-mark" onclick="deleteMarker(' +
    L.stamp(marker) +
...
function saveMarkerInfo(event, markerId) {
  ...
  let data = {
    ...
    popupContent:
      ...
      description +
      "</p><button class='popup-button-delete delete-mark' onclick=deleteMarkFromDB(" +
      markerId +
      ")>Eliminar</button>",
  };
...
document.getElementById("search-button").addEventListener("click", function () {
  let cityName = document.getElementById("city-search").value.trim();
    ...
    fetch(
      ...
      .then((data) => {
        if (data && data.length > 0) {
          ...
          //Centro el mapa en las coordenadas
          map.setView([lat, lon], 12);
        } else {
          Swal.fire({
            icon: "error",
            title: "Oops...",
            text: "Lo siento, no se encontro ese lugar.",
          });
        }
      });
  }
